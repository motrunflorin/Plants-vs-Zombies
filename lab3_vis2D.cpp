...
void Lab3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        float zoomFactor = 0.98f; 
        float oldWidth = logicSpace.width;
        float oldHeight = logicSpace.height;
        logicSpace.width *= zoomFactor;
        logicSpace.height *= zoomFactor;
      
        logicSpace.x += (oldWidth - logicSpace.width) / 2;
        logicSpace.y += (oldHeight - logicSpace.height) / 2;
    }

    if (window->KeyHold(GLFW_KEY_X)) {
        float zoomFactor = 1.02f; 
        float oldWidth = logicSpace.width;
        float oldHeight = logicSpace.height;
        logicSpace.width *= zoomFactor;
        logicSpace.height *= zoomFactor;
      
        logicSpace.x -= (logicSpace.width - oldWidth) / 2;
        logicSpace.y -= (logicSpace.height - oldHeight) / 2;
    }
